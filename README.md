...
#  Подготовка обучающей выборки на Python
Данные взяты [от сюда](https://www.kaggle.com/c/titanic/data)
...
#### Посмотрим на наши входные данные
...
#### Посмотрим на список признаков (колонок) которые у нас есть
...
* `PassengerId` - уникальный индетификатор каждого человека
...
* `Parch` - количество прямых родственников на борту
* `Ticket` - номер билета
...
#### Изучим какие значения принимает каждое поле
...
Итак, почти все значения (кроме `Name`, `PassengerId`, `Ticket`, но о них позже) изучены. Теперь самое время начать обрабатывать данные

## Предварительная обработка

##### Заполним отсутствующие значения возраста медианой возрастов
...
#####  Заполним отсутствующие значения порта самым популярным портом
...
##### Уберем лишние поля
`Name`, `PassengerId`, `Ticket` - эти значения являются просто идентификатором и по сути в обучении нам никаки не помогут, поэтому хорошей идеей будет дропнуть их. 
Заодно дропнем `Cabin`, из-за малого количества заполненных значений
